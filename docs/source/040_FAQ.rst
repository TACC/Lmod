...
How does the module command work?
   ...
   Note that the $LMOD_CMD writes message to stderr because anything
   written to stderr bypasses the eval.  Whereas all text written to
   stdout is evaluated by the **eval** command.
