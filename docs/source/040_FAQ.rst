...
How does the module command work?
   ...
   parents' environment but not the other way around.  The module
   command is a two part process.  The module shell function in bash
   is::
   ...
   /apps/lmod/lmod/libexec/lmod).  So if you have a module file (foo/1.0) that
   contains::
...
etc doing it in the environment?
    ...
    environment.  To prevent problem the various shells, the table is
...
do?
   ...
   If this does find it then you might have a personal spider cache.
   To clear it do::
   ...
   cache.  If you are the system administrator then read
