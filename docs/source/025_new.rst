...
**myModuleFullNameAndAlias()**
  (Lmod X.Y.Z+) Modulefiles can call ``myModuleFullNameAndAlias()`` to obtain
  the logical full name and the true loaded full name when dot-leading path
  segments are used to hide modules.  See :ref:`dot_hidden_load_alias-label`
  and :ref:`lua_modulefile_functions-label`.  Rationale:
  `Lmod issue #849 <https://github.com/TACC/Lmod/issues/849>`__.
=======
**Dot-hidden load alias search for NVV paths and meta modules**
  (Lmod X.Y.Z+) When ``LMOD_DOT_HIDDEN_LOAD_ALIAS`` is ``yes``, dot-hidden
  alias resolution applies to any path segment (including NVV layouts) and to
  meta-module short names, subject to the normalized uniqueness rule described
  in :ref:`dot_hidden_load_alias-label`.  See `Lmod issue #849
  <https://github.com/TACC/Lmod/issues/849>`__.
>>>>>>> 32059c8b7 (Extend dot-hidden load alias search to NVV paths and meta modules (IS849))

**Support for Package Managers to have their own SitePackage.lua files**
  (Lmod 9.2.8+) Lmod now support the env. var. LMOD_SITEPACKAGE_PREPEND.
  This is a colon separated list of absolute paths to Lua files.
  These files are loaded into Lmod before /etc/lmod/lmod_config.lua
  and  SitePackage.lua.
  
**module_virtual()** **and Tcl module-virtual**
   Lua ``.modulerc.lua`` files support ``module_virtual ("virtual_name,
   "implementation")``, and Tcl ``.modulerc`` files support ``module-virtual``,
   matching Environment Modules style virtual names that map to one hidden
   implementation module. Virtual names appear under their modulepath in
   ``module avail`` (not under ``Global Aliases``). See :ref:`modulerc-label`. Rationale:
   `Lmod issue #818 <https://github.com/TACC/Lmod/issues/818>`__.

**setenv() and pushenv() are no longer unset inside the Lmod process on module unload**
  (Lmod 9.2.6+) On module unload, **setenv()** and **pushenv()** will
  be reversed on the user process but not the Lmod process.

**The env. variable LMOD_DISABLE_SAME_NAME_AUTOSWAP deprecated**
  (Lmod 9.2.6+) Please use LMOD_DISABLE_NAME_AUTOSWAP instead.

**Terse output for module show with inherit**
  (Lmod 9.2.5+) *module --terse show* now reports inherited modulefile
  paths with ``-->`` lines.  See :ref:`terse_output-label` for more
  details.

**Lmod now ignores module names that have leading or trailing spaces characters**
  (Lmod 9.2.3+) Module now ignores modules with leading or trailing
  space characters.  Lmod will now report any modules with this issue
  by using the command **$LMOD_DIR/check_module_tree_syntax**.


**Lmod now protects itself from changes to LD_LIBRARY_PATH and LD_PRELOAD**
  (Lmod 9.2.3+) For the shells bash, zsh, t/csh and the python
  interface, Lmod protects itself from changes to LD_LIBRARY_PATH and
  LD_PRELOAD.  Modules can modify these environment variables but Lmod
  now internally uses the configure time values.

**Lmod now looks for /etc/lmod/SitePackage.lua**
  (Lmod 9.0.7+): After looking for SitePackage.lua in the usual
  places, Lmod now looks in /etc/lmod for SitePackage.lua.  Actually
  Lmod looks in LMOD_CONFIG_DIR which defaults to /etc/lmod

**New commands** *hideRegex* {} **and** *forbidRegex* {} **have been added**
  (Lmod 9.0.6+) The modulerc files now support Lua based regular
  expression matching.  See :ref:`modulerc-label` for more details.


**Collection are written to ~/.config/lmod only**
   (Lmod 9.0+) This means that Lmod will only write to ~/.config/lmod
   but always read from both directories (~/.config/lmod and ~/.lmod.d)
