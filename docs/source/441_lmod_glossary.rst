...
.. glossary::
   ...
   M.load()
       A method of the ``MainControl`` object (often aliased as ``M`` or ``mcp``), defined in ``src/MainControl.lua``. This function is a key part of the module loading sequence, typically called after more specific user request processing (like ``M.load_usr()`` or other ``mcp`` states). Its primary responsibility is to orchestrate the actual loading of a prepared list of ``MName`` objects (``mA``). It achieves this by:
       1.  Acquiring a singleton instance of the ``hub``.
       2.  Calling ``hub:load(mA)``, which then executes the detailed loading logic, rule enforcement, and modulefile evaluation managed by the ``hub`` (whose logic is in ``src/Hub.lua``).
       It may also handle ancillary tasks like registering admin messages.

   M.load_usr()
       A method of the ``MainControl`` object (aliased as ``M`` or ``mcp``), defined in ``src/MainControl.lua``. This function is specifically responsible for handling module load requests that originate directly from a user command (e.g., ``module load foo``). It is called by ``Load_Usr()`` (from ``src/cmdfuncs.lua``).
       Its key actions include:
       1.  Registering the list of modules (``mA``, an array of ``MName`` objects) that the user has requested to load (e.g., by calling ``l_registerUserLoads(mA)`).
       2.  Calling ``self:load(mA)`` (which is ``M.load()``) to pass the list of modules to the next stage of the loading process, which involves the ``hub``.
       3.  Registering a dependency check (``self:registerDependencyCk()``) to be performed after the loading operations.

   M.prepend_path()
       A method of the ``MainControl`` object (aliased as ``M`` or ``mcp``), defined in ``src/MainControl.lua``. This is called when a modulefile uses ``prepend_path()`` during a load operation.
       1.  It retrieves the current ``varT`` (variable table) via ``FrameStk:singleton():varT()``.
       2.  It ensures a ``Var`` object exists in ``varT`` for the path-like variable name (``argT[1]``), configured with the appropriate delimiter (e.g., ":", from ``argT.delim``) and duplicate handling policy (``argT.nodups``).
       3.  It calls the ``:prepend(value, nodups, priority)`` method on this ``Var`` object, using the path element from ``argT[2]``. This adds the new path element to the beginning of the variable's value in ``varT``.

   M.remove_path()
       A method of the ``MainControl`` object (aliased as ``M`` or ``mcp``), defined in ``src/MainControl.lua``. This is called when a modulefile's ``prepend_path()`` (or ``append_path()``) effect needs to be undone during an unload operation.
       1.  It retrieves the current ``varT``.
       2.  It ensures a ``Var`` object exists in ``varT`` for the path-like variable name (``argT[1]``).
       3.  It calls a method like ``:remove(value)`` on this ``Var`` object, using the path element from ``argT[2]``, to remove that specific path element from the variable's value in ``varT``.

   M.setenv()
       A method of the ``MainControl`` object (aliased as ``M`` or ``mcp``), defined in ``src/MainControl.lua``. It's invoked when a modulefile uses the ``setenv()`` command during a load operation.
       1.  It retrieves the current ``varT`` (variable table) via ``FrameStk:singleton():varT()``.
       2.  It ensures a ``Var`` object exists in ``varT`` for the environment variable name specified in ``argT[1]``.
       3.  It calls the ``:set(value)`` method on this ``Var`` object, passing the value from ``argT[2]``. This updates Lmod's internal representation of the environment variable.
       4.  It can optionally ``respect`` an existing environment variable value if ``argT[3]`` is true and the variable is already set in the environment.

   M.unsetenv()
       A method of the ``MainControl`` object (aliased as ``M`` or ``mcp``), defined in ``src/MainControl.lua``. It's invoked when a modulefile uses ``setenv()`` during an unload operation (or ``unsetenv()`` directly, though this is less common for direct ``M.unsetenv`` mapping from modulefile ``unsetenv``).
       1.  It retrieves the current ``varT``.
       2.  It ensures a ``Var`` object exists in ``varT`` for the variable name (``argT[1]``).
       3.  It calls the ``:unset()`` method on this ``Var`` object, marking the variable for removal or reversion in Lmod's internal environment representation.
       4.  It can also ``respect`` the current value if ``argT[3]`` (respect) is true and the current environment value differs from ``argT[2]`` (the value originally set).
   ...
   myGlobal
       Refers to the Lua script ``src/myGlobals.lua``. This script is ``require``d by nearly all other Lmod Lua modules and its primary purpose is to initialize and make available a wide range of global variables, internal constants, and fundamental settings that govern Lmod's runtime behavior.
       Key aspects:
   ...
   prepend_path()
       A Lua function available for use within Lmod modulefiles (defined in ``src/modfuncs.lua``). Its purpose is to prepend a new directory path to a path-like environment variable (e.g., ``PATH``, ``LD_LIBRARY_PATH``).
       When a modulefile calls ``prepend_path("VAR_NAME", "/new/path", {delim=":", nodups=true/false, priority=N})``:
       1.  It interacts with the current ``mcp`` (Main Control Program) object.
       2.  If Lmod is in a "loading" context, ``prepend_path()`` will typically delegate to ``mcp:prepend_path({name, value, nodups, priority, delim})``, which in turn calls ``M.prepend_path()`` in ``src/MainControl.lua`` to modify the internal ``varT`` by adding the path.
       3.  If Lmod is in an "unloading" context, it will typically delegate to ``mcp:remove_path(...)``, which calls ``M.remove_path()`` to ensure the previously added path is removed from ``varT``.
       The optional third argument is a table for specifying a custom delimiter, whether duplicates are allowed (defaulting to Lmod's general duplicate path settings), and a priority.
   ...
   setenv()
       A Lua function available for use within Lmod modulefiles (defined in ``src/modfuncs.lua``). Its purpose is to set or modify an environment variable.
       When a modulefile calls ``setenv("VAR_NAME", "value", {respect=true/false})``:
       1.  It interacts with the current ``mcp`` (Main Control Program) object.
       2.  If Lmod is in a "loading" context, ``setenv()`` will typically delegate to ``mcp:setenv({name, value, respect})``, which in turn calls ``M.setenv()`` in ``src/MainControl.lua`` to set the variable's value in the internal ``varT``.
       3.  If Lmod is in an "unloading" context, it will typically delegate to ``mcp:unsetenv(...)``, which calls ``M.unsetenv()`` to revert or remove the variable from ``varT``.
       The optional third argument ``respect`` (defaults to false) can be used to indicate that if the variable is already set in the environment, its existing value should be preserved.
